...
def index():
    ...
    team = requests.post('http://service-4:5003/Draft Pick',json={'position': position, 'pick': pick}).text
    
    # new_pick = MLBDraft(team=team,position=position,pick=pick)
    # db.session.add(new_pick)
    # db.session.commit()

    return render_template('index.html', picks=all_picks, team=team, pick=pick, position=position)
